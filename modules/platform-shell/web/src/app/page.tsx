...
const voxObservatoryUrl =
  process.env.NEXT_PUBLIC_VOX_OBSERVATORY_URL ?? "http://localhost:3006";
const coopManagerUrl =
  process.env.NEXT_PUBLIC_COOP_MANAGER_URL ?? "http://localhost:3003";
...
const modules: PlatformModule[] = [
  {
    ...
    path: `${voxObservatoryUrl}/dashboard`,
    enabled: true,
    port: 3006,
    icon: "monitor",
  ...
  },
  {
    id: "coop-manager",
    name: "Coop Manager",
    description: "Gestão de cooperados, unidades e dashboard operacional.",
    path: `${coopManagerUrl}/dashboard`,
    enabled: true,
    port: 3003,
    icon: "coop",
    healthEndpoint: "http://localhost:3004/api/health",
    docsUrl: "http://localhost:3004/docs/",
  },
];
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const healthChecks = modules.map(async (module) => {
      if (!module.healthEndpoint) {
        return null;
      }

      try {
        const response = await fetch(module.healthEndpoint, {
        ...
        try {
          ...
          if (text) {
            ...
            // Se a API retornar um status, garantimos que seja um indicativo de sucesso
            if (
              data?.status &&
              !["active", "ok", "up", "online"].includes(
          ...
          // Se a resposta for 200 OK, mas não for um JSON válido (ex: res.send("OK")), consideramos online.
        }
        ...
        return { id: module.id, status: "inactive" } as const;
      }
    });

    Promise.all(healthChecks)
      .then((results) => {
        const updates = results.reduce<Record<string, HealthStatus>>(
          (acc, item) => {
            if (item) {
        ...
        setModuleHealth((current) => ({ ...current, ...updates }));

        const allActive = results.every(
          (item) => item === null || item.status === "active",
        );

        setHealth(allActive ? "active" : "inactive");
      })
      .catch(() => {
        setHealth("inactive");
      });
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl bg-slate-800 p-4">
                      ...
                      <p className="mt-2 font-semibold text-slate-200">
                        {module.healthEndpoint
                          ? moduleHealth[module.id] === "loading"
                            ? "..."
                            : moduleHealth[module.id] === "active"
                              ? "Online"
                              : "Offline"
                          : module.enabled
                            ? "Ativo"
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {module.docsUrl ? (
                      <Button
                        ...
                        variant="secondary"
                        size="lg"
                        className="w-full shadow-lg shadow-slate-500/10 sm:w-auto"
                      >
                        <Link
                          href={module.docsUrl}
                          target="_blank"
